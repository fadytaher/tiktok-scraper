...
export class TikTokScraper extends EventEmitter {
  ...
  constructor({
    ...
    this.filepath = process.env.SCRAPING_FROM_DOCKER
      ? "/usr/app/files"
      : filepath || "";
    ...
    this.historyPath = process.env.SCRAPING_FROM_DOCKER
      ? "/usr/app/files"
      : historyPath || tmpdir();
    this.idStore = "";
    ...
    this.Downloader = new Downloader({
      ...
      noWaterMark,
      headers,
      filepath: process.env.SCRAPING_FROM_DOCKER
        ? "/usr/app/files"
        : filepath || "",
      bulk
    });
  ...
  private get fileDestination(): string {
    ...
    switch (this.scrapeType) {
      ...
      case "hashtag":
        ...
        return this.filepath
          ? `${this.filepath}/${this.input}_${Date.now()}`
          : `${this.input}_${Date.now()}`;
      default:
        if (!this.zip && this.download) {
          return `${this.folderDestination}/${this.scrapeType}_${Date.now()}`;
        }
        return this.filepath
          ? `${this.filepath}/${this.scrapeType}_${Date.now()}`
          : `${this.scrapeType}_${Date.now()}`;
    }
  }
  ...
  private get folderDestination(): string {
    switch (this.scrapeType) {
      ...
      case "music":
        return this.filepath
          ? `${this.filepath}/music_${this.input}`
          : `music_${this.input}`;
      case "trend":
  ...
  private request<T>(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const simpleOptions = {
        jar: this.cookieJar,
        uri: signature
          ? `${unsignedUrl}&_signature=${signature}`
          : uri || this.input,
      ...
      if (this.scrapeType === "user") {
        ...
        if (appContext && appContext.text) {
          ...
          let data = Object.keys(itemModule).map(k => {
            let i = itemModule[k];
            let authorInfos =
              userModule.users[Object.keys(userModule.users)[0]];
            let authorStats =
              userModule.stats[Object.keys(userModule.stats)[0]];
            return {
              itemInfos: i,
              authorInfos,
              authorStats
  ...
  public async scrape(): Promise<Result | any> {
    ...
    if (this.download && !this.zip) {
      try {
        await fromCallback(cb =>
          mkdir(this.folderDestination, { recursive: true }, cb)
        );
      } catch (error) {
    ...
    if (!this.scrapeType || CONST.scrape.indexOf(this.scrapeType) === -1) {
      return this.returnInitError(
        `Missing scraping type. Scrape types: ${CONST.scrape} `
      );
    }
      ...
      forEachLimit(
        ...
        async (item: PostCollector) => {
          try {
            ...
            item.videoUrlNoWaterMark = await this.getUrlWithoutTheWatermark(
              item.videoApiUrlNoWaterMark!
            );
          } catch {
        ...
        (item, cb) => {
          switch (this.scrapeType) {
            case "user":
              this.getUserId()
                .then(query =>
                  this.submitScrapingRequest(
                    { ...query, cursor: this.maxCursor },
                    true
                  )
                ) //, cursor: this.maxCursor
   ...
   */
  private async submitScrapingRequest(
    query: RequestQuery,
    updatedApiResponse = false
  ): Promise<boolean> {
    try {
      ...
      if (
        (!result.itemListData && updatedApiResponse && !result.itemList) ||
        (!updatedApiResponse && !result.items)
      ) {
        throw new Error("No more posts");
      }

      const { done } = await this.collectPosts(
        result.itemListData ? result.itemListData : result.itemList
      );
      ...
      this.maxCursor = parseInt(
        maxCursor === undefined ? cursor : maxCursor,
        10
      );
      return false;
    } catch (error) {
  ...
  public async saveMetadata({ json, csv }) {
    if (this.collector.length) {
      switch (this.filetype) {
        case "json":
          await fromCallback(cb =>
            writeFile(json, JSON.stringify(this.collector), cb)
          );
          break;
        case "csv":
          await fromCallback(cb =>
            writeFile(csv, this.json2csvParser.parse(this.collector), cb)
          );
          break;
        case "all":
          await Promise.all([
  ...
  private async getDownloadedVideosFromHistory() {
    try {
      const readFromStore = (await fromCallback(cb =>
        readFile(
          `${this.historyPath}/${this.storeValue}.json`,
          { encoding: "utf-8" },
          cb
        )
      )) as string;
  ...
  private async storeDownloadProgress() {
    const historyType =
      this.scrapeType === "trend"
        ? "trend"
        : `${this.scrapeType}_${this.input}`;
    const totalNewDownloadedVideos = this.collector.filter(
    ...
    if (this.storeValue && totalNewDownloadedVideos) {
      ...
      try {
        const readFromStore = (await fromCallback(cb =>
          readFile(
            `${this.historyPath}/tiktok_history.json`,
            { encoding: "utf-8" },
            cb
          )
        )) as string;
        ...
        item = Object.assign(
          ...
          ),
          {
            ...
            hashtags: post.itemInfos.challengeInfoList
              ? post.itemInfos.challengeInfoList.map(
                  ({ id, title, desc, coverLarger }) => ({
                    id,
                    name: title,
                    title: desc,
                    cover: coverLarger
  ...
  private async scrapeData<T>(qs: RequestQuery): Promise<T> {
    this.storeValue =
      this.scrapeType === "trend"
        ? "trend"
        : qs.id || qs.challengeID! || qs.musicID!;

    const unsignedURL = `${this.getApiEndpoint}?${new URLSearchParams(
      qs as any
    ).toString()}`;
    ...
    try {
      const response = await this.request<T>(
        options,
        true,
        this.scrapeType == "user" ? true : false,
        unsignedURL,
        await _signature
      );
      return response;
  ...
  public async getMusicInfo(): Promise<MusicMetadata> {
    ...
    const query = {
      uri: `https://www.tiktok.com/node/share/music/${
    ...
    const unsignedURL = `${query.uri}?${new URLSearchParams(
      query.qs as any
    ).toString()}`;
  ...
  private async getVideoMetadataFromHtml(): Promise<FeedItems> {
    ...
    try {
      ...
      const rawVideoMetadata_1 = response.split(
  ...
  private async getVideoMetadata(url = ""): Promise<FeedItems> {
    ...
    if (videoData) {
      ...
      try {
        const response = await this.request<VideoMetadata>(
          requestOptions,
          false,
          true
        );
  ...
  public async getVideoMeta(html = true): Promise<PostCollector> {
    ...
    try {
      if (this.noWaterMark) {
        ...
        videoItem.videoUrlNoWaterMark = await this.getUrlWithoutTheWatermark(
      ...
      forEachLimit(
        ...
        (item, cb) => {
          rp({
            ...
            ...(this.method === "GET"
              ? { qs: { json: encodeURIComponent(JSON.stringify(item)) } }
              : {}),
            json: true
          })
            .then(() => {
